...
export const DeferredMenuFilter = ({
    ...
    shaderChunks: {
      ...
      pass: /*glsl*/ `
        ...
        if(uMenuStrength > 0.) {
          ...
          if (uStylizeEnabled && dfm_luma <= 0.5 && dfm_depth <= 0.99) {
          ...
          if (uStylizeEnabled && dfm_luma <= 0.8 && dfm_depth <= 0.99) {
            if (mod((vUv.x + dfm_displacement.x) * uResolution.y + (vUv.y + dfm_displacement.y) * uResolution.x, uStylizeMod + fract(uTime * 0.1)) <= uStylizeThickness) {
              dfm_color = vec4(uStylizeColor, 1.0);
            };
          }
        ...
        // pc_fragColor.rgb = vec3(uMenuStrength, 0.0, 0.0);
      `,
    },
  });
  ...
  const bwMultiplier = useAppStore((state) => state.bwMultiplier);
  useEffect(() => {
    refDeferred.current.uniforms.uMenuStrength.value = bwMultiplier;
  }, [bwMultiplier]);
  ...
  useEffect(() => {
    refDeferred.current.uniforms.uPosterizeEnabled.value = posterize.enabled;
    refDeferred.current.uniforms.uPosterizeGamma.value = posterize.gamma;
    refDeferred.current.uniforms.uPosterizeNumBands.value = posterize.numBands;
  }, [posterize.enabled, posterize.gamma, posterize.numBands]);

  useEffect(() => {
    refDeferred.current.uniforms.uStylizeEnabled.value = stylize.enabled;
    refDeferred.current.uniforms.uStylizeMod.value = stylize.mod;
    refDeferred.current.uniforms.uStylizeThickness.value = stylize.thickness;
    refDeferred.current.uniforms.uStylizeColor.value = _stylizeColor;
  }, [stylize.enabled, stylize.mod, stylize.thickness, _stylizeColor]);

  useEffect(() => {
    refDeferred.current.uniforms.uColorizeEnabled.value = colorize.enabled;
    refDeferred.current.uniforms.uColorizeBrightness.value =
      colorize.brightness;
    refDeferred.current.uniforms.uColorizeContrast.value = colorize.contrast;
    refDeferred.current.uniforms.uColorizeColor.value = _colorizeColor;
  }, [
    colorize.enabled,
    colorize.brightness,
    colorize.contrast,
