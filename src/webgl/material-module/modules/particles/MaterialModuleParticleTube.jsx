...
export const MaterialModuleParticleTube = ({ count = 1 }) => {
  ...
  const tIndices = suspend(async () => {
    ...
    dt.needsUpdate = true;
    return dt;
  }, [`${_key}-MaterialModuleParticleTube-${dataSize}`]);

  useEffect(() => {
    return () => {
      tIndices.dispose();
      clear([`${_key}-MaterialModuleParticleTube-${dataSize}`]);
    };
  }, [tIndices]);

  useMaterialModule({
    name: 'MaterialModuleParticleTube',
    uniforms: {
      uThickness: { value: 1 },
