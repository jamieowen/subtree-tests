import { forwardRef, useMemo } from 'react';

import { urls } from '@/config/assets';

import quaternion from '@/webgl/glsl/utils/quaternion.glsl';
import transformUV from '@/webgl/glsl/utils/transformUV.glsl';
import range from '@/webgl/glsl/utils/range.glsl';
import quaternionToMatrix4 from '@/webgl/glsl/utils/quaternionToMatrix4.glsl';

export const CloudRingMaterial = forwardRef(
  ({ dataSize, children, tMap, ...props }, ref) => {
    const tNoise = useTexture(urls.t_noise_simplex);
    tNoise.wrapS = tNoise.wrapT = RepeatWrapping;

    // UNIFORMS
    const uniforms = useMemo(() => {
      return {
        uTime: { value: 0 },
        uDelta: { value: 0 },
        tNoise: { value: tNoise },
        tMap: { value: tMap },
      };
    }, []);

    const shader = useMemo(() => {
      return {
        transparent: true,
        // depthTest: false,
        // depthWrite: true,

        uniforms,
        vertexShader: /*glsl*/ `
          precision mediump float;

          attribute vec2 reference;
          attribute float index;

          varying vec2 vUv;
          varying vec3 vNormal;
          flat varying vec2 vReference;
          varying float vIndex;
          varying vec3 vWorldPos;
          varying float vProgress;
          varying vec3 vPosition;
          varying vec4 vRand;

          uniform float uTime;

          uniform sampler2D uLifeTexture;
          uniform sampler2D uVelocityTexture;
          uniform sampler2D uPositionTexture;
          uniform sampler2D uRandomTexture;
          uniform sampler2D uRotationTexture;

          ${quaternionToMatrix4}

          void main() {

            vec4 life = texture2D(uLifeTexture, reference);
            vProgress = life.y;

            vec3 pos = position;

            // Scale
            // modelPosition *= clamp(vProgress, 0.0, 1.0);
            
            // Rotate
            vec4 rot = texture2D(uRotationTexture, reference);
            // pos *= quaternionToMatrix4(rot);

            

            vec4 rand = texture2D(uRandomTexture, reference);

            // Translate
            vec4 posTex = texture2D(uPositionTexture, reference);
            pos *= 1. + rand.z;
            pos += posTex.xyz;
            

            gl_Position = projectionMatrix * modelViewMatrix * vec4(pos, 1.);
            vNormal = normalize(normalMatrix * normal);

            vUv = uv;
            vRand = rand;
            vWorldPos = pos;
            vReference = reference;
            vIndex = index;
            vPosition = pos;
          }
        `,
        fragmentShader: /*glsl*/ `
          varying vec2 vUv;
          varying vec3 vWorldPos;
          varying vec3 vPosition;
          varying vec3 vNormal;
          varying vec4 vRand;
          varying float vProgress;
          varying float vIndex;
          flat varying vec2 vReference;

          uniform float uTime;
          uniform sampler2D tNoise;
          uniform sampler2D tMap;
          

          vec3 direction = vec3(1,1,4);
          float distortion = 0.3;
          float layers = 3.;
          float shades = 4.;
          float speed = 0.01;

          // const float PI = 3.141592;

          layout(location = 1) out vec4 gNormal;
          layout(location = 2) out vec4 gWorldPos;

          // float gyroid (vec3 seed) { 
          // return dot(sin(seed),cos(seed.yzx)); 
          //   //return dot(sin(seed),cos(seed.xyz)); 
          // }

          // float fbm (vec3 seed) {
          //     float result = 0., a = .5;
          //     for (int i = 0; i < 7; ++i, a /= 2.) {		
          //         seed += direction * uTime * speed*.01/a;
                  
          //         float cloudMovement = .001;
          //         //seed.z += uTime * cloudMovement;
          //         seed.z += result*distortion;                
                  
          //         result += gyroid(seed/a)*a;
          //     }
          //     return result;
          // }

          // float aastep(float threshold, float value) {
          //     // float afwidth = length(vec2(dFdx(value), dFdy(value))) * 0.70710678118654757;
          //     float afwidth = fwidth(value) * 0.5;
          //     return smoothstep(threshold-afwidth, threshold+afwidth, value);
          // }

          // float aastep(float threshold, float value, float padding) {
          //     return smoothstep(threshold - padding, threshold + padding, value);
          // }

          // vec2 aastep(vec2 threshold, vec2 value) {
          //     return vec2(
          //         aastep(threshold.x, value.x),
          //         aastep(threshold.y, value.y)
          //     );
          // }

          ${transformUV}
          ${quaternion}
          ${range}

          //	Classic Perlin 3D Noise 
          //	by Stefan Gustavson
          //
          // vec4 permute(vec4 x){return mod(((x*34.0)+1.0)*x, 289.0);}
          // vec4 taylorInvSqrt(vec4 r){return 1.79284291400159 - 0.85373472095314 * r;}
          // vec3 fade(vec3 t) {return t*t*t*(t*(t*6.0-15.0)+10.0);}

          // float cnoise(vec3 P){
          //   vec3 Pi0 = floor(P); // Integer part for indexing
          //   vec3 Pi1 = Pi0 + vec3(1.0); // Integer part + 1
          //   Pi0 = mod(Pi0, 289.0);
          //   Pi1 = mod(Pi1, 289.0);
          //   vec3 Pf0 = fract(P); // Fractional part for interpolation
          //   vec3 Pf1 = Pf0 - vec3(1.0); // Fractional part - 1.0
          //   vec4 ix = vec4(Pi0.x, Pi1.x, Pi0.x, Pi1.x);
          //   vec4 iy = vec4(Pi0.yy, Pi1.yy);
          //   vec4 iz0 = Pi0.zzzz;
          //   vec4 iz1 = Pi1.zzzz;

          //   vec4 ixy = permute(permute(ix) + iy);
          //   vec4 ixy0 = permute(ixy + iz0);
          //   vec4 ixy1 = permute(ixy + iz1);

          //   vec4 gx0 = ixy0 / 7.0;
          //   vec4 gy0 = fract(floor(gx0) / 7.0) - 0.5;
          //   gx0 = fract(gx0);
          //   vec4 gz0 = vec4(0.5) - abs(gx0) - abs(gy0);
          //   vec4 sz0 = step(gz0, vec4(0.0));
          //   gx0 -= sz0 * (step(0.0, gx0) - 0.5);
          //   gy0 -= sz0 * (step(0.0, gy0) - 0.5);

          //   vec4 gx1 = ixy1 / 7.0;
          //   vec4 gy1 = fract(floor(gx1) / 7.0) - 0.5;
          //   gx1 = fract(gx1);
          //   vec4 gz1 = vec4(0.5) - abs(gx1) - abs(gy1);
          //   vec4 sz1 = step(gz1, vec4(0.0));
          //   gx1 -= sz1 * (step(0.0, gx1) - 0.5);
          //   gy1 -= sz1 * (step(0.0, gy1) - 0.5);

          //   vec3 g000 = vec3(gx0.x,gy0.x,gz0.x);
          //   vec3 g100 = vec3(gx0.y,gy0.y,gz0.y);
          //   vec3 g010 = vec3(gx0.z,gy0.z,gz0.z);
          //   vec3 g110 = vec3(gx0.w,gy0.w,gz0.w);
          //   vec3 g001 = vec3(gx1.x,gy1.x,gz1.x);
          //   vec3 g101 = vec3(gx1.y,gy1.y,gz1.y);
          //   vec3 g011 = vec3(gx1.z,gy1.z,gz1.z);
          //   vec3 g111 = vec3(gx1.w,gy1.w,gz1.w);

          //   vec4 norm0 = taylorInvSqrt(vec4(dot(g000, g000), dot(g010, g010), dot(g100, g100), dot(g110, g110)));
          //   g000 *= norm0.x;
          //   g010 *= norm0.y;
          //   g100 *= norm0.z;
          //   g110 *= norm0.w;
          //   vec4 norm1 = taylorInvSqrt(vec4(dot(g001, g001), dot(g011, g011), dot(g101, g101), dot(g111, g111)));
          //   g001 *= norm1.x;
          //   g011 *= norm1.y;
          //   g101 *= norm1.z;
          //   g111 *= norm1.w;

          //   float n000 = dot(g000, Pf0);
          //   float n100 = dot(g100, vec3(Pf1.x, Pf0.yz));
          //   float n010 = dot(g010, vec3(Pf0.x, Pf1.y, Pf0.z));
          //   float n110 = dot(g110, vec3(Pf1.xy, Pf0.z));
          //   float n001 = dot(g001, vec3(Pf0.xy, Pf1.z));
          //   float n101 = dot(g101, vec3(Pf1.x, Pf0.y, Pf1.z));
          //   float n011 = dot(g011, vec3(Pf0.x, Pf1.yz));
          //   float n111 = dot(g111, Pf1);

          //   vec3 fade_xyz = fade(Pf0);
          //   vec4 n_z = mix(vec4(n000, n100, n010, n110), vec4(n001, n101, n011, n111), fade_xyz.z);
          //   vec2 n_yz = mix(n_z.xy, n_z.zw, fade_xyz.y);
          //   float n_xyz = mix(n_yz.x, n_yz.y, fade_xyz.x); 
          //   return 2.2 * n_xyz;
          // }

          // // Mimics blender node
          // float subtractWithFactor(float value1, float value2, float factor) {
          //     float result = value1 - value2;  // Simple subtraction
          //     return mix(value1, result, factor);  // Blend between original value and subtracted result
          // }

          void main() {            
            // // Step 1: uv to object coords
            vec3 pos = vPosition;

            
            float fov = 0.1;

            // // Bigger value means smaller circle
            // float innerRadius = 0.1;
            
            // // Step 2: add z by 0.5 (0.5 too high, add 0.05 instead)
            // // Lower value means clouds rotate more
            pos.z += fov;

            // // Step 3: normalize
            pos = normalize(pos);
            
            // // Step 4: animate z offset (subtract)
            pos.z += uTime * 0.1 + (vRand.z * 0.1);
            
            float angleInfluence = -2. * vRand.x;
            float angle = (1. - length(pos));
            
            // vec4 q = quatFromAxisAngle(vec3(0., 0., 1.), angle * angleInfluence);
            // vec3 rotated = rotateVectorByQuaternion(q, pos);
            vec2 rotated = rotateUV(vUv, angle * angleInfluence);

            vec4 noiseTex = texture2D(tNoise, rotated.xy * 2.);
            vec4 noiseTexBig = texture2D(tNoise, rotated.xy);
            // vec3 noisePos = vec3(rotated);
            // float noise = cnoise(noisePos * 4.01);
            // // float noise = fbm(noisePos * 1.01);

            vec4 tex = texture2D(tMap, vUv);

            float layers = 3.;
            float shades = 4.;
            float gradient = fract(tex.r * layers);
            float shade = round(gradient * shades) / shades;

            
            // vec3 color = mix(tint * gradient, tint2, shade);
            //vec3 color = mix(tint, tint2, border);
            

            // float innerCircle = subtractWithFactor(length(pos.xy), 0.2, gradient);
            // innerCircle = pow(smoothstep(0.5, 0.6, innerCircle), 2.);

            vec2 uv = vUv;

            vec2 center = vec2(0.5);
            float l1 = length(uv - center);

            float progress = (1. - vProgress);
            float p = crange(progress, 0., 0.1, 0., 1.);

            // float thickness = 0.05 + noiseTex.r * 0.3;
            float thickness = (0.15 + vRand.x * 0.1);
            thickness *= noiseTexBig.r;
            // thickness = 0.;
            
            float radius = (0.2 + vRand.y * 0.1);
            radius = mix(radius, radius * noiseTexBig.r, 0.5);
            //radius *= max(radius * 0.9, 1. - progress);

            float radius2 = radius + thickness;
            float b1 = step(radius, l1);
            float b2 = step(l1, radius2);

            // float a = b1 * b2 * step(0.5, noiseTexBig.b);
            float a = b1 * b2;

            float t1 = crange(vRand.r, 0., 1., 0.4, 0.7);
            vec3 tint = vec3(t1) * crange(progress, 0., 1., 0.5, 1.);
            // vec3 tint2 = vec3(0.318, 0.506, 0.545) * 0.5;

            float dep = crange(vWorldPos.z, -2., 5., 0., 1.);
            gl_FragDepth = 1. - dep;


            if (a < 0.1) discard;
            vec3 final = tint * noiseTexBig.r;
            // float alpha = a * p;
            float alpha = a;
            pc_fragColor = vec4(1.0 - final, alpha);
            
            
            gNormal = vec4(vNormal, 1.0);
            gWorldPos = vec4(vWorldPos, 1.0);            
          }
        `,
      };
    }, []);

    useFrame((state, delta) => {
      uniforms.uTime.value = state.clock.getElapsedTime();
      uniforms.uDelta.value = delta;
    });

    return (
      <shaderMaterial
        ref={ref}
        args={[shader]}
      />
    );
  }
);
